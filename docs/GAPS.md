...
**Shipped:** `subagent.failed` sets the `fail` status, so the `x` glyph is reachable from real data for the first time; the failure's `model` is recovered when the spawn event omitted one; a failed count is rendered on the workspace row; the row says `failed` in words rather than only in a tooltip; and the `error` text is never published.
**Corrected once in use.** Failed rows were first shipped exempt from BOTH retention and dismissal, on the reasoning that a failure must never be quietly hidden. That was wrong, and the first report was "they aren't going away and I don't understand what they are": six failures from a finished Session sat on screen permanently with no way to clear them. Permanent and unclearable is not importance - it is noise that cannot be turned off, and it buries the failures that are still new. Failures now age out on the operator's own retention window and are dismissible individually and in bulk. Running work remains unhideable, which is the rule that actually matters.
