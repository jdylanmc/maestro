...
state-digest: 15b8821d093076c176d199062333bde5c1e0af564a0879d1bbd18f816a6f52f7
root-map-digest: 5f0aca06475f70f67d67a75542185e08714d22a1d4c228d5a5530fcdf7d58304
root-lexicon-digest: 7e776bcfef5345bdf38a3763c32047ab69852c6c2cdcdf6b071a631ee0dfd7df
...
| Attention | confirmed | A Session observed to want its human. | Maestro observation | AT_RISK (discouraged) | [`CONTEXT.md`](../../../../CONTEXT.md) | maestro-graphical-agent-orchestrator/n-0000/c-0010 | c-0029 | Session, Liveness | session:maestro-graphical-agent-orchestrator |
...
- Divergence: Issue #1 is stale behind c-0025 and canonical ADR 0004. It still describes a standalone orchestrator, Fleet/worktree enforcement, durable lifecycle ownership, four implementation routes, and the Copilot SDK as the universal seam. The promoted tracker identity is retained; reconciliation belongs to `/discovery`.
...
- Open questions: (1) Replace the 8 MiB tail read with incremental event-log reads; current behavior silently misses 73.4% of subagents in large logs. (2) Bind the exact Session instead of choosing the newest log for a shared working directory. (3) Remove `preToolUse` and add the complete hook kill switch. (4) Re-cut publication away from the transitional workspace description. (5) Treat cmux hook-store identity, live lifecycle, and restore as incomplete until duplicate records, idle-only state, and the generated `-C` restore failure are resolved.
...
| --- | --- | --- | --- | --- | --- |
| n-0011 | decision-ready | promotion-ready | P0 | none | Complete reads, exact Session identity, remove `preToolUse`, add the kill switch, and replace the transitional description publisher. |
| n-0013 | decision-ready | researched | P0 | none | Replacement tree channel, task-list presentation, and editor hand-off file. |
| n-0014 | decision-ready | researched | P0 | none | Reliable live and terminal-state clearing; native cmux stayed `idle` during active work. |
| n-0016 | decision-ready | researched | P0 | none | Duplicate native records, failed restore, migrated uninstall, and unobserved `needsInput`/`unknown`. |
...
n-0008 and n-0009 remain historical tracker concerns rather than current
product fog.
...
| --- | --- | --- | --- | --- | --- |
| n-0000 | Branch | maestro-graphical-agent-orchestrator/n-0000 | [Issue #1](https://github.com/jdylanmc/maestro/issues/1) | c-0019 | Open in c-0029 - tracker still describes the retired standalone orchestrator and Fleet domain. |
...
- Open questions: (1) Replace the transitional single-line workspace-description wire format with proven per-tab channels or another representation without losing parentage, state, or activity. (2) Present the Session task list now that `log` is proven retained and queryable but not workspace-card content. (3) Choose the editor hand-off file when only the folder is unambiguous. Platform facts are settled: the sidebar renders end to end, a running row uses the spinner, a failed row uses the red `xmark`, and `cmux sidebar open maestro` produces a real resizable pane.
- Evidence: [c-0026](./cycles/c-0026.md); [c-0029](./cycles/c-0029.md); [cmux custom sidebar authoring contract](https://raw.githubusercontent.com/manaflow-ai/cmux/main/docs/custom-sidebars.md); [cmux CLI contract](https://raw.githubusercontent.com/manaflow-ai/cmux/main/docs/cli-contract.md); live c-0029 accessibility-tree verification of the spinner, failure mark, hierarchy, and split pane
...
- Maturity: researched
- Priority: P0
- Outcome: Every terminal running a Copilot Session carries a visible per-tab status. Maestro prefers cmux's native lifecycle when it is measured to work and publishes a fail-open fallback where it does not. c-0029 proved status and progress render, but also proved the native hook store remained `idle` throughout active tool work, so native lifecycle cannot yet be the sole source for `working`.
- Open questions: (1) Establish reliable clearing when a Session dies without an idle event. (2) Determine whether `initializing` is a distinct visible state or part of `working`. (3) Re-measure `needsInput` and `unknown`; c-0029 observed neither. The workspace lane push remains retired; per-tab status is the correct granularity.
- Evidence: [c-0027](./cycles/c-0027.md); [c-0029](./cycles/c-0029.md) - 16,089 lifecycle observations stayed `idle` during a ten-second active tool call; the Attention predicate measured firing in [c-0012](./cycles/c-0012.md) and confirmed on cmux in [c-0024](./cycles/c-0024.md); [cmux CLI contract](https://raw.githubusercontent.com/manaflow-ai/cmux/main/docs/cli-contract.md)
...
- Open questions: The install and coexistence questions are settled by c-0029: after a backup, cmux's hooks installed, Copilot migrated them from JSONC `config.json` to `settings.json`, and both hook sources fired without denying tools. Remaining: (1) explain or canonicalize the many store records that share one process and transcript; (2) fix and re-measure Copilot restore after the generated `-C` argument failure; (3) verify uninstall after migration; (4) observe `needsInput` and `unknown`. Maestro's own hook entry remains necessary for the subagent tree and live-working fallback. It must mirror `CMUX_COPILOT_HOOKS_DISABLED=1`, and `preToolUse` should be removed because it retains observer-veto risk without producing a native `running` transition.
