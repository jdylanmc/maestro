...
- Maturity: researched
- Priority: P0
- Outcome: Every terminal running a Copilot Session carries a visible state, published as a **per-tab cmux status pill** (`set-status --color --tab=<surface-id>`) so it renders in Maestro's sidebar and the stock sidebar alike. Subagents never publish state; only the terminal-level agent does. **The c-0028 sweep found that cmux infers this same lifecycle natively once its Copilot hooks are installed, so Maestro's derivation becomes a fallback rather than the source.**
- Open questions: (1) ~~Own badges, or cmux's native lane?~~ **Settled in c-0027: both.** Maestro renders per-terminal state at the correct granularity and also pushes the workspace lane. (2) ~~Push the native lane at all?~~ **RETIRED in c-0028.** The lane push is withdrawn one cycle after it was decided, on evidence that a strictly better channel already existed: `cmux set-status --tab=<surface-id>` is a status pill **with a colour, per terminal**, which is the correct granularity and renders in the stock sidebar as well. The workspace lane would have been a third, coarser writer of the same fact - able to stomp a manually pinned lane, unable to name which of two agents needs the operator, and liable to disagree with the lifecycle cmux infers natively once its Copilot hooks are installed. The c-0027 most-urgent-wins aggregation was sound reasoning about the wrong channel. (2b) ~~How is the lane resolved when a workspace holds several agents?~~ **Settled: most-urgent-wins**, `needs-attention` > `working` > `done`. Any agent blocked on a permission request makes the workspace `needs-attention`; otherwise any agent mid-turn makes it `working`; otherwise `done`. Last-writer-wins was rejected because it makes the lane lie, which is the failure class this plugin cannot afford. (3) ~~What overrides a lane the operator pinned by hand?~~ **Settled: `needs-attention` only.** Maestro stays out of a manual pin except when an agent is genuinely blocked on the operator, which is the one case worth interrupting for. (4) ~~What counts as finished?~~ **Settled: `agent_idle`.** (5) **Unresolved:** whether `initializing` maps onto the `working` lane or the `todo` lane; cmux infers `todo` from its own checklist, so writing it may fight the inference. (6) **Unresolved:** how a state is cleared when a session dies without an `agent_idle` - the c-0025 stale-description defect in a new place.
...
- Open questions: (1) **The install is unmeasured and touches the file that broke this environment three times.** `cmux hooks setup copilot` writes `~/.copilot/config.json`. Every prior break in this session came from a hook entry in Copilot's own hook configuration - a non-zero exit or any output denies every tool call. *Mitigation:* back up the file, install, verify a full session survives a tool call, and keep a one-command rollback. **Not yet run.** (2) What cmux's integration does **not** carry: it records a lifecycle of `running`, `idle`, `needsInput`, `unknown` per surface, and no subagent information of any kind. The tree stays Maestro's. (3) Whether Maestro's own hook entry and cmux's can coexist in one `~/.copilot/config.json`, or whether Maestro's becomes redundant and should be removed. (4) Session restore arrives as a **consequence**: `copilot --resume <id>` is re-run on relaunch from `~/.cmuxterm/copilot-hook-sessions.json`. That is materially the durable-lifecycle capability c-0025 retired as the largest build item, now inherited rather than built.
- Evidence: [c-0028](./cycles/c-0028.md); [cmux agent-hooks contract](https://raw.githubusercontent.com/manaflow-ai/cmux/main/docs/agent-hooks.md) - `copilot` is one of sixteen supported agents, binary `copilot`, installed file `~/.copilot/config.json`, session restore `copilot --resume <id>`, Feed bridge `PreToolUse`; the session store records agent session id, cmux workspace id, surface id, cwd, pid, sanitized launch command, and lifecycle. **Not installed on this machine** - `~/.cmuxterm/` holds only `events.jsonl` and `~/.copilot/config.json` has no `hooks` key - so every claim here is documented, not observed
- Links: refines n-0011; informs n-0013, n-0014
- First seen: c-0028
- Former node id: none
- Reinterpreted: c-0028 (created)
- Promotion key: none
- Tracker: none
- Divergence: none
